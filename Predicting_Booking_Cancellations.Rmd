...
#some outliers here from there on out - children, babies, adults, repeated guest, previous cancellations, booking changes etc. 
...
#the average of babies is 0.01 - propose to merge it with children
...
In this section we also set up the recipe, since this remains (almost) the same across the models. We are including all the independent variables we have in our data post-exploratory analysis and feature engineering, as predictors in our models. Additionally:

-    we standardize the numerical variables to reduce the effect of outliers, and to be able to perform KNN classification (since the variable scale matters for the distance measurement)

-   we create dummy variables from all our categorical variables with one-hot encoding so there is a dummy variable created for each class of these variables, which is a more useful representation for decision trees.

To deal with the class imbalance, here we chose to also test a recipe with downsampled data (based on the variable we want to predict) version.
...
```{r}
# these are metrics we defined above
...
We choose to go with the model, which has the highest accuracy, which is model2 with 6 random predictors being chosen at each split. Now we test the performance of this model on the "validation" set we created above in Model Assessment Setup. As mentioned, the labels for the actual test set (`bookings_test_solutions.RData`) were not provided until later on in the project so this was a necessary step for us.

```{r}
# select the best parameters based on the accuracy metric
best_acc <- select_best(rf_tune_res_ds, metric = "accuracy")
...
For communication with stakeholders, we plot out the variable importance here, as determined by the random forest algorithm:

```{r}
# here we plot out the importance of the features as determined by our random forest algorithm
...
At this point, we were provided the labels for the test dataset and could test the performance of our tuned model on "real-life" data.

## Cleaning dataset

Same steps as were applied to our training dataset are replicated here.
